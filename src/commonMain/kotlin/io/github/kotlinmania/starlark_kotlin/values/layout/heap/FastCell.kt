 ...
 *
 * In Kotlin, GC handles memory. We use a nullable wrapper to simulate
 * the init/uninit state of Rust's `UnsafeCell<MaybeUninit<T>>`.
 */
internal class FastCell<T>(
     ...
     *
     * In Kotlin, tracked implicitly by [value] != null,
     * but we keep an explicit flag to mirror the Rust structure.
     */
    ...
    fun take(): T {
        ...
        @Suppress("UNCHECKED_CAST")
        val v = value as T
        // Replace the value field with null so that accessing it will crash.
        value = null
        return v
    }
