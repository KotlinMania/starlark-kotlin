...
// Placeholder types that need to be ported from their respective modules

internal expect class Freezer
internal expect class Private
internal expect class Methods
internal expect object MethodsStatic {
    fun methods(builder: (MethodsBuilder) -> Unit): Methods
}
internal expect class MethodsBuilder
internal expect class Heap {
    fun alloc(value: String): Value
    fun allocStr(value: String): StarlarkStr
    fun allocStrConcat(a: String, b: String): StarlarkStr
}
internal expect class Value {
    fun unpackStarlarkStr(): StarlarkStr?
    fun unpackStr(): String?
    fun unpackI32(): Int?
    fun toValue(): Value
}
internal expect class StarlarkHashValue {
    fun get(): Int
    companion object {
        fun newUnchecked(value: Int): StarlarkHashValue
    }
}
internal expect class StarlarkHasher {
    fun write(bytes: ByteArray)
    fun writeU32(value: Int)
    fun finishSmall(): StarlarkHashValue
}
internal expect class Hashed<T> {
    companion object {
        fun <T> newUnchecked(hash: StarlarkHashValue, value: T): Hashed<T>
    }
}
internal expect class AlignedPaddedStr {
    companion object {
        fun new(len: Int, body: Array<Long>): AlignedPaddedStr
    }
}
internal expect class NoneOr<T> {
    fun intoOption(): T?
    companion object {
        fun <T> None(): NoneOr<T>
        fun <T> Other(value: T): NoneOr<T>
    }
}
internal expect class CharIndex {
    val value: Int
    constructor(value: Int)

    operator fun compareTo(other: CharIndex): Int
    operator fun minus(other: CharIndex): CharIndex
}
internal expect class StrIndices {
    val start: CharIndex
    val haystack: String
}
internal expect object fastString {
    fun at(s: String, index: CharIndex): Char?
    fun len(s: String): CharIndex
    fun contains(haystack: String, needle: String): Boolean
    fun convertStrIndices(s: String, start: Int?, stop: Int?): StrIndices?
}
internal expect object interpolation {
    fun percent(format: String, value: Value): Result<String>
}
internal expect object ValueError {
    fun unsupportedWith(self: Any, op: String, other: Value): Result<Nothing>
    fun unsupported(op: String, value: Value): Result<Nothing>
    fun invalidIndex(index: Value): Result<Nothing>
    class IndexOutOfBound(index: Int) : Exception
    class InvalidType(value: Value, expected: String) : Exception
}
internal expect class Ty {
    companion object {
        inline fun <reified T> starlarkValue(): Ty
    }
}
internal expect class Serializer {
    fun serializeStr(value: String): Result<Unit>
}
internal expect fun <T> applySlice(
    list: List<T>,
    start: Value?,
    stop: Value?,
    stride: Value?
): Result<List<T>>
internal expect fun stringRepr(s: String, buffer: StringBuilder)
