...
package io.github.kotlinmania.starlark_kotlin.collections.string_pool
...
internal class StringPool {
    ...
    private val strings: MutableList<StringBuilder> = mutableListOf()
    ...
    fun alloc(): StringBuilder {
        val sb = if (strings.isNotEmpty()) {
            strings.removeAt(strings.lastIndex)
        } else {
            StringBuilder()
        }
        require(sb.isEmpty()) { "Pooled string must be empty" }
        return sb
    }
