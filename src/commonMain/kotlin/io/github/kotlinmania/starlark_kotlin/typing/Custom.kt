...
class TyCustom(
    ...
    companion object {
        fun <T : TyCustomImpl> new(impl: T): TyCustom = TyCustom(TyCustomDynBridge(impl))

        fun union2(x: TyCustom, y: TyCustom): kotlin.Result<TyCustom> {
            val result = x.dyn.union2Dyn(y.dyn)
            return if (result.isSuccess) {
                kotlin.Result.success(TyCustom(result.getOrThrow()))
            } else {
                kotlin.Result.failure(result.exceptionOrNull()
                    ?: IllegalArgumentException("Cannot merge custom types"))
            }
        }

        fun intersects(x: TyCustom, y: TyCustom): Boolean = x.dyn.intersectsDyn(y.dyn)
    }

    fun asName(): String? = dyn.asNameDyn()

    fun asFunctionDyn(): TyFunction? = dyn.asFunctionDyn()

    fun asCallableDyn(): TyCallable? = dyn.asCallableDyn()

    fun validateCallDyn(args: TyCallArgs): Result<Ty> = dyn.validateCallDyn(args)

    fun isIntersectsWithDyn(other: TyBasic): Boolean = dyn.isIntersectsWithDyn(other)

    fun attributeDyn(attr: String): Result<Ty> = dyn.attributeDyn(attr)

    fun iterItemDyn(): Result<Ty> = dyn.iterItemDyn()

    fun indexDyn(item: TyBasic): Result<Ty> = dyn.indexDyn(item)

    fun binOpDyn(binOp: TypingBinOp, rhs: TyBasic): Result<Ty> = dyn.binOpDyn(binOp, rhs)

    fun intersectsWith(other: TyBasic): Result<Boolean> {
        if (dyn.isIntersectsWithDyn(other)) {
            return Result.success(true)
        }
        return when (other) {
            is TyBasic.Custom -> Result.success(intersects(this, other.custom))
            is TyBasic.Callable -> {
                val thisCallable = dyn.asCallableDyn()
    ...
    fun matcherWithTypeCompiledFactory(factory: TypeMatcherFactory<Any>): Any {
        return dyn.matcherWithTypeCompiledFactory(factory)
    }

    fun matcherWithBox(): TypeMatcherBox {
        return dyn.matcherBoxDyn()
    }

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is TyCustom) return false
        return dyn.eqToken() == other.dyn.eqToken()
    }

    override fun hashCode(): Int = dyn.hashCodeDyn()

    override fun toString(): String = dyn.toString()

    override fun compareTo(other: TyCustom): Int {
        val (aCmp, aTypeName) = dyn.cmpToken()
        val (bCmp, bTypeName) = other.dyn.cmpToken()

        // Type ids are comparable, but we want comparison independent of hashing.
        if (aCmp::class != bCmp::class) {
        ...
        @Suppress("UNCHECKED_CAST")
        val aOrd = aCmp as Comparable<Any>
        return aOrd.compareTo(bCmp)
    }
}
