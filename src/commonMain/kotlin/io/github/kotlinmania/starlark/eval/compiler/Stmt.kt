...
internal sealed class AssignModifyLhs {
    ...
    data class Array(
        val expr: IrSpanned<ExprCompiled>,
        val index: IrSpanned<ExprCompiled>,
    ) : AssignModifyLhs()

    data class Local(
        val slot: IrSpanned<LocalSlotId>,
    ) : AssignModifyLhs()

    data class LocalCaptured(
        val slot: IrSpanned<LocalCapturedSlotId>,
    ) : AssignModifyLhs()

    data class Module(
        val slot: IrSpanned<ModuleSlotId>,
    ) : AssignModifyLhs()
}
...
internal sealed class AssignCompiledValue {
    ...
    data class Index(
        val array: IrSpanned<ExprCompiled>,
        val index: IrSpanned<ExprCompiled>,
    ) : AssignCompiledValue()

    data class Tuple(
        val elements: List<IrSpanned<AssignCompiledValue>>,
    ) : AssignCompiledValue()
    ...
    val assign =
        when (val node = expr.node) {
            ...
            is AssignTargetP.Identifier<*, *> -> {
                ...
                val bindingId =
                    (node.ident.node.payload as BindingId?)
