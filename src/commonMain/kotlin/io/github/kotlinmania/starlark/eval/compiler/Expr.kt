...
internal sealed class ExprCompiled {
    ...
    data class Local(
        val slot: LocalSlotId,
    ) : ExprCompiled()

    /** Read local captured variable. */
    data class LocalCaptured(
        val slot: LocalCapturedSlotId,
    ) : ExprCompiled()
    ...
    data class TupleExpr(
        val elements: List<IrSpanned<ExprCompiled>>,
    ) : ExprCompiled()
    ...
    data class DictExpr(
        val entries: List<Pair<IrSpanned<ExprCompiled>, IrSpanned<ExprCompiled>>>,
    ) : ExprCompiled()

    /** Comprehension. */
    data class Compr(
        val compr: ComprCompiled,
    ) : ExprCompiled()
    ...
    data class Call(
        val call: IrSpanned<CallCompiled>,
    ) : ExprCompiled()
    ...
    internal fun asType(): IrSpanned<ExprCompiled>? =
        when (this) {
    ...
    internal fun asTypeIs(): Pair<IrSpanned<ExprCompiled>, FrozenStringValue>? =
        when (this) {
            is Builtin1Expr -> {
                val builtin = this.op
    ...
    internal fun asBuiltinValue(): FrozenValue? =
        when (this) {
    ...
    internal fun isIterableEmpty(): Boolean =
        when (this) {
     ...
     */
    private fun isDefinitelyBool(): Boolean =
        when (this) {
     ...
     * - has no effects
     */
    internal fun isPureInfallible(): Boolean =
        when (this) {
            is ValueExpr -> true
    ...
    internal fun asLocalNonCaptured(): LocalSlotId? =
        when (this) {
            is Local -> slot
            else -> null
        }
...
private sealed class ExprShortList {
    class Exprs(
        val exprs: List<IrSpanned<ExprCompiled>>,
    ) : ExprShortList()

    class Constants(
        val constants: List<FrozenValue>,
    ) : ExprShortList()
}
...
internal fun IrSpanned<ExprCompiled>.isPureInfallibleToBool(): Boolean? = node.isPureInfallibleToBool()
...
internal sealed class EvalError(
    override val message: String,
) : Exception(message) {
    class DuplicateDictionaryKey(
        key: String,
    ) : EvalError("Dictionary key repeated for `$key`")
}
...
private fun <P : AstPayload> ExprP<P>.unpackStringLiteral(): String? =
    when (this) {
        is ExprP.Literal -> {
...
internal fun Compiler.exprTruth(
    expr: CstExpr,
): Result<IrSpanned<ExprCompiledBool>> = this.expr(expr).map { ExprCompiledBool.new(it) }
