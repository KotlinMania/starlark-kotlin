...
/**
 * In Rust this module is called `__derive_refs`. In Kotlin we keep the role but follow Kotlin
 * package naming conventions (`deriverefs`).
 *
 * This module allows us to reference other crates in starlark_derive without users needing to be
 *  aware of those dependencies. We make them public here and then can reference them like
 *  `starlark::__derive_refs::foo` in Rust.
 */

// Submodules:
// pub mod components    -> deriverefs (Components.kt)
// pub mod invoke_macro_error -> deriverefs (InvokeMacroError.kt)
// pub mod param_spec    -> deriverefs (ParamSpec.kt)
// pub mod parse_args    -> deriverefs (ParseArgs.kt)
// pub mod sig           -> deriverefs (Sig.kt)
