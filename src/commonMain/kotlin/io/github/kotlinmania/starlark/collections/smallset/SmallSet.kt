...
class SmallSet<T> private constructor(
    ...
    fun containsHashed(key: Hashed<T>): Boolean = entries.any { it == key }

    fun <Q> containsHashedEquivalent(key: Hashed<Q>): Boolean where Q : Equivalent<T> {
        val q = key.key()
    ...
    fun <Q> shiftRemoveHashedEquivalent(value: Hashed<Q>): Boolean where Q : Equivalent<T> {
        val q = value.key()
