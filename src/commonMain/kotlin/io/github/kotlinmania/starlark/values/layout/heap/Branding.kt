 ...
 *
 * ```rust,ignore
 * # import crate::values::Heap;
 * # import crate::values::Value;
 * # import crate::values::tuple::AllocTuple;
 * Heap::temp(|heap1| {
 *     Heap::temp(|heap2| {
 *         let s1: Value<'_> = heap1.allocStr("abc").toValue();
 ...
 * dependent frozen heaps alive). We make import of the expanded contract by then providing an API
 * like this:
 *
 * ```rust,ignore
 * implementation<'v> Heap<'v> {
 *     fun accessOwnedFrozenValue(self, v: OwnedFrozenValue) -> Value<'v>;
 * }
 * ```
 *
 * `accessOwnedFrozenValue` adds the heap the frozen value is associated with as a dependency of
