...
internal fun <P : AstPayload> ParameterP<P>.toSourceString(): String =
    when (this) {
        ...
        is ParameterP.Normal -> {
            ...
            if (typ != null) {
                val typeNode = typ.node.expr.node
                val typeExpr = typeNode.toSourceString()
                sb
                    .append(": ")
                    .append(typeExpr)
            }
            if (defaultVal != null) {
                sb
                    .append(" = ")
                    .append(defaultVal.node.toSourceString())
            }
            sb.toString()
        }
...
internal fun <P : AstPayload> ExprP<P>.toSourceString(): String =
    when (this) {
        ...
        is ExprP.Call -> {
            val sb = StringBuilder()
            sb
                .append(expr.node.toSourceString())
                .append("(")
        ...
        is ExprP.Slice -> {
            ...
            if (step != null) {
                sb
                    .append(":")
                    .append(step.node.toSourceString())
            }
            sb.append("]")
            sb.toString()
        }
        ...
        is ExprP.Dict -> {
            val elementsSource =
                commaSeparatedFmt(elements, false) {
                    "${it.first.node.toSourceString()}: ${it.second.node.toSourceString()}"
                }
            "{$elementsSource}"
        }
        is ExprP.ListComprehension -> {
            ...
            sb
                .append("[")
                .append(expr.node.toSourceString())
            val targetNode = forClause.varTarget.node
            val overNode = forClause.over.node
            sb
                .append(" for ")
                .append(targetNode.toSourceString())
                .append(" in ")
                .append(overNode.toSourceString())
            for (c in clauses) {
                when (c) {
                    is ClauseP.For -> {
                        val targetNode = c.forClause.varTarget.node
                        val overNode = c.forClause.over.node
                        val targetSource = targetNode.toSourceString()
                        val overSource = overNode.toSourceString()
                        sb
                            .append(" for ")
                            .append(targetSource)
                            .append(" in ")
        ...
        is ExprP.DictComprehension -> {
            ...
            sb
                .append("{")
                .append(key.node.toSourceString())
                .append(": ")
                .append(value.node.toSourceString())
            val targetNode = forClause.varTarget.node
            val overNode = forClause.over.node
            sb
                .append(" for ")
        ...
        is ExprP.FString -> {
            val sb = StringBuilder()
            sb
                .append(fstring.node.format.node)
                .append(".format(")
...
internal fun <P : AstPayload> StmtP<P>.toSourceString(tab: String = ""): String =
    when (this) {
        ...
        is StmtP.Assign -> {
            ...
            sb
                .append(tab)
                .append(assign.lhs.node.toSourceString())
            if (assign.ty != null) {
                val typeNode = assign.ty.node.expr.node
                val typeExpr = typeNode.toSourceString()
                sb
                    .append(": ")
                    .append(typeExpr)
            ...
            sb
                .append(" = ")
                .append(assign.rhs.node.toSourceString())
                .append("\n")
            sb.toString()
        }
        ...
        is StmtP.If -> {
            ...
            sb
                .append(tab)
                .append("if ")
                .append(cond.node.toSourceString())
                .append(":\n")
        ...
        is StmtP.For -> {
            ...
            sb
                .append(tab)
                .append("for ")
                .append(forStmt.varTarget.node.toSourceString())
                .append(" in ")
                .append(forStmt.over.node.toSourceString())
                .append(":\n")
        ...
        is StmtP.Def<*, *> -> {
            ...
            sb
                .append(tab)
                .append("def ")
                .append(def.name.node.ident)
                .append("(")
            sb.append(commaSeparatedFmt(def.params, false) { it.node.toSourceString() })
            sb.append(")")
            if (def.returnType != null) {
                val returnTypeNode = def.returnType.node.expr.node
                val returnType = returnTypeNode.toSourceString()
                sb
                    .append(" -> ")
                    .append(returnType)
            }
        ...
        is StmtP.Load<*, *> -> {
            ...
            for (arg in loadStmt.args) {
                sb
                    .append(", ")
                    .append(arg.local.node.ident)
                    .append(" = ")
                sb
                    .append('"')
                    .append(arg.their.node)
                    .append('"')
            }
            sb.append(")\n")
            sb.toString()
