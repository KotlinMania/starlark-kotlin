...
internal fun testCanConvert() {
    ...
    val v = Value("test")
    val v2 = Value2("test")
    check(convertValue(v) == v)
    check(convertAny(v) == v)
    check(convertAny(v2) == null)
}

// #[test]
// fn test_any_lifetime() {
internal fun testAnyLifetime() {
...
internal fun testProvidesStaticTypeId() {
    // fn test<'a, A: AnyLifetime<'a>>(expected: TypeId)
    fun test(expected: KClass<*>, provider: ProvidesStaticType) {
        check(expected == provider.staticType)
    }
