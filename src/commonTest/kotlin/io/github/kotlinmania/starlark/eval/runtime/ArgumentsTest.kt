...
import io.github.kotlinmania.starlark.collections.symbol.Symbol.Companion.new
import io.github.kotlinmania.starlark.eval.runtime.ArgNames.Companion.newCheckUnique
...
private fun stringValue(heap: Heap, value: String): StringValue = StringValue.new(heap.allocStr(value)) ?: error("Expected string value")
...
class ArgumentsTest {
    ...
    fun testParameterNoNamed() {
        Heap.temp { heap ->
            ...
            p.full.names = newCheckUnique(listOf(new("test") to stringValue(heap, "test"))).getOrThrow()
            assertTrue(p.noNamedArgs().isFailure)
            assertEquals(1, p.len().getOrThrow())
        }
    }
    ...
    fun testNamesMapRepeatedNameInArgNames() {
        Heap.temp { heap ->
            ...
            assertTrue(newCheckUnique(names).isFailure)
        }
    }
}
