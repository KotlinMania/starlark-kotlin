...
package io.github.kotlinmania.starlark.debug.adapterimpl
...
import io.github.kotlinmania.starlark.debug.Variable.Companion.truncateString
...
private class BreakpointController {
    ...
    fun getClient(): DapAdapterClient = Client(this)
...
private fun evalWithHook(
    ...
    return Module.withTempHeap { env ->
        ...
        env
            .freeze()
            .getOrElse { error("error freezing module") }
            .get("_")
            .getOrElse { error("missing _") }
...
internal fun testTruncateString() {
    check("Hello" == truncateString("Hello", 10))
    check("Hello" == truncateString("Hello", 5))
    // A string that should be truncated at a character boundary
    check("Hello, ...(truncated)" == truncateString("Hello, \u4E16\u754C", 7))
    // A string that would be truncated within a multi-byte character
    check("Hello, ...(truncated)" == truncateString("Hello, \u4E16\u754C", 8))
    // A string that should be truncated just before a multi-byte character
    check("Hello, ...(truncated)" == truncateString("Hello, \u4E16\u754C", 9))
    check("Hello, \u4E16...(truncated)" == truncateString("Hello, \u4E16\u754C", 10))
}
