...
private data class Value(val value: String) : ProvidesStaticType, AnyLifetime {
...
private data class Value2(val value: String) : ProvidesStaticType, AnyLifetime {
...
private class TypeCarrier(private val type: KClass<*>) : ProvidesStaticType, AnyLifetime {
...
private class Bbb(val value: String) : ProvidesStaticType {
    override val staticType: KClass<*> get() = Bbb::class
}

private class Ccc<X>(val value: X) : ProvidesStaticType {
    override val staticType: KClass<*> get() = Ccc::class
}

private class LifetimeTypeConst<T>(val values: List<T>) : ProvidesStaticType {
    override val staticType: KClass<*> get() = LifetimeTypeConst::class
}

private class TypeWithConstraint<T : CharSequence>(val value: T) : ProvidesStaticType {
    override val staticType: KClass<*> get() = TypeWithConstraint::class
}

private class TypeWhichDoesNotImplementAnyLifetime

private class TypeWithStaticLifetime<T>(val value: T) : ProvidesStaticType {
    override val staticType: KClass<*> get() = TypeWithStaticLifetime::class
}

private interface My

private class FooBar<P : My>(val value: P) : ProvidesStaticType {
